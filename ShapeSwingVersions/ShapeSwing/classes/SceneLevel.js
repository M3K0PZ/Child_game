                ...
                case 1:
                    ...
                    this.physics.add.overlap(this.ball, this.star, (ball, collectible) => {
                        ...
                        if (this.ball.id == 1) {
                            ...
                            this.time.delayedCall(100, () => { //comme un thread en section critique
...
function trajectoire(graphics, startPoint, pointer, ball) {
    graphics.clear(); //enlever les anciens points s'ils existent
    ...
    var currentDistance = Phaser.Math.Distance.Between(startPoint.x,
        startPoint.y,
        endPoint.x,
        endPoint.y);
    // distance entre le point de départ et le point actuel
    var angle = Phaser.Math.Angle.Between(startPoint.x,
        startPoint.y,
        endPoint.x,
        endPoint.y);
    // angle entre le point de départ et le point actuel
...
function shoot(ball, coef, distance, angle) {
    ...
    // cap la vitesse de la balle
    if (velocity > 850) {
        velocity = 850;
    }
    let velocityX = Math.cos(angle) * velocity;
    let velocityY = Math.sin(angle) * velocity;
    ball.setVelocity(-velocityX, -velocityY); // set la vitesse de la balle
}
